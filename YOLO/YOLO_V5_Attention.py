...
transform = transforms.Compose([
    transforms.Resize((640, 640)),  # YOLO typically uses 640x640 input
...
# Model save path
model_save_path = r"D:\Research\Topics\Propagranate\model\yolo\Yolo5_model.pth"
...
# BottleneckCSP with CBAM (modified from YOLOv5)
class BottleneckCSP(nn.Module):
    def __init__(self, in_channels, out_channels, n=1, shortcut=True):
        ...
        self.conv3 = nn.Conv2d(hidden_channels * 2, out_channels, 1, 1, bias=False)
...
# SPP (Spatial Pyramid Pooling) with CBAM
class SPP(nn.Module):
...
# YOLOv5 Backbone with Attention
class YOLOv5Backbone(nn.Module):
    def __init__(self, num_classes=3):
        super(YOLOv5Backbone, self).__init__()
        # Input: 3x640x640
        self.focus = Focus(3, 32)  # 32x320x320
        self.conv1 = nn.Conv2d(32, 64, 3, 2, 1, bias=False)  # 64x160x160
        self.csp1 = BottleneckCSP(64, 64, n=1)  # 64x160x160
        self.conv2 = nn.Conv2d(64, 128, 3, 2, 1, bias=False)  # 128x80x80
        self.csp2 = BottleneckCSP(128, 128, n=3)  # 128x80x80
        self.conv3 = nn.Conv2d(128, 256, 3, 2, 1, bias=False)  # 256x40x40
        self.csp3 = BottleneckCSP(256, 256, n=3)  # 256x40x40
        self.conv4 = nn.Conv2d(256, 512, 3, 2, 1, bias=False)  # 512x20x20
        self.spp = SPP(512, 512)  # 512x20x20
        self.csp4 = BottleneckCSP(512, 512, n=1, shortcut=False)  # 512x20x20
        
        # Transformer attention
        self.transformer = SelfAttention(512)
...
model = YOLOv5Backbone(num_classes=num_classes).to(device)
...
# Grad-CAM implementation for YOLOv5
def grad_cam(input_image, model, target_class):
    ...
    # Target layer - using the last CSP layer
    target_layer = model.csp4.bottlenecks[-1].conv2
...
def shap_explanation(input_image, model, target_class):
    ...
    shap_heatmap = cv2.resize(shap_heatmap, (640, 640))  # Resize to match the input image size
