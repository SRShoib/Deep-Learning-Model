...
model_save_path = r"D:\Research\Topics\Black_Gram_Dataset\Mendeley_Data\Model Training\Model\attention_mobilenetv2_model.pth"
...
# Define the MobileNetV2 with Attention Model
class MobileNetV2Attention(nn.Module):
    def __init__(self, num_classes, pooling_type):
        super(MobileNetV2Attention, self).__init__()
        # Load pretrained MobileNetV2
        base_model = models.mobilenet_v2(weights=models.MobileNet_V2_Weights.IMAGENET1K_V1)
        
        # Remove the classifier and get the features
        self.features = base_model.features
        
        # MobileNetV2's last feature map has 1280 output channels
        self.attention = nn.Sequential(
            nn.Conv2d(1280, 256, kernel_size=1),  # Reduce channels first
            nn.ReLU(),
        ...
        self.classifier = nn.Sequential(
            nn.Linear(1280, 512),  # MobileNetV2's last feature map has 1280 channels
            nn.ReLU(),
...
model = MobileNetV2Attention(num_classes=num_classes, pooling_type=pooling_type).to(device)
